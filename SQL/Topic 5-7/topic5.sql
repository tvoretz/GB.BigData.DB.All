...
mysql> SELECT u.* FROM users AS u JOIN orders AS o ON u.id = o.user_id GROUP BY u.id;
+----+-------------+-------------+---------------------+---------------------+
| id | name        | birthday_at | created_at          | updated_at          |
+----+-------------+-------------+---------------------+---------------------+
|  6 | Wilton      | 1993-10-01  | 2012-03-18 23:46:21 | 1975-01-31 16:08:25 |
...
mysql> SELECT u.id, u.name, COUNT(*) as qty FROM users AS u JOIN orders AS o ON u.id = o.user_id GROUP BY u.id;
+----+-------------+-----+
| id | name        | qty |
+----+-------------+-----+
|  6 | Wilton      |   2 |
...
mysql> SELECT f.id, c1.name AS RU_from, c2.name AS RU_to FROM flights AS f JOIN cities AS c1 ON f.from_city = c1.label JOIN cities AS c2 ON f.to_city = c2.label ORDER BY f.id;
+------+------------------+----------------+
| id   | RU_from          | RU_to          |
+------+------------------+----------------+
|    1 | Москва           | Омск           |
